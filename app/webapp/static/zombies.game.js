// app/webapp/static/zombies.core.js  [ULTRA LUX COD-ZOMBIES-2D v1.4 | 3D-READY CORE MAX++]
...
// FIX MAX++: ultra-robust roguelike mode detection (no more "UI says roguelike but core is arcade"),
// guaranteed economy progression in roguelike (baseline coins on kill + drops + wave rewards),
// stable snapshot API, safer clamps, optional CloudStorage hooks-ready (no UI change required).
(() => {
  "use strict";
  ...
  // -------------------------
  // Mode helpers (HARD FIX)
  // -------------------------
  const _modeNorm = (m) => String(m || "").toLowerCase().replace(/[^a-z]/g, "");
  const _isRogueStr = (m) => _modeNorm(m).includes("rogue") || _modeNorm(m).includes("roguelike");
  function isRogue(CORE) {
    // Super-robust: accept both meta.mode, state.mode, and loose strings like "ROGUELIKE • Astra"
    const mm = CORE?.meta?.mode;
    const sm = CORE?.state?.mode;
    return _isRogueStr(mm) || _isRogueStr(sm);
  }
  ...
  const CORE = {
    ...
    state: {
      ...
      // HARD: mirror mode in state too (fixes "UI says rogue but core thinks arcade")
      mode: "arcade",
    ...
    start(mode, w, h, opts = {}, tms = _now()) {
      const wantRogue = _isRogueStr(mode) || _isRogueStr(opts?.mode);

      this.meta.mode = wantRogue ? "roguelike" : "arcade";
      this.state.mode = this.meta.mode;
    ...
    // HARD: explicit mode switch (keeps run, for debugging / UI mismatch protection)
    setMode(mode) {
      const wantRogue = _isRogueStr(mode);
      this.meta.mode = wantRogue ? "roguelike" : "arcade";
      this.state.mode = this.meta.mode;

      // if switching into roguelike mid-run, enable economy immediately (no reset)
      if (wantRogue) {
        this.state.coins = Math.max(0, this.state.coins | 0);
        this.state.level = Math.max(1, this.state.level | 0);
    ...
    updateFrame(tms) {
      ...
      // HARD: if renderer/UI accidentally overwrote meta.mode, keep state.mode mirror consistent
      if (_isRogueStr(this.meta.mode) !== _isRogueStr(this.state.mode)) {
        this.state.mode = this.meta.mode;
      }
    ...
    getFrameData() {
      ...
      return {
        alpha: a,
        meta: { ...this.meta, mode: rogue ? "roguelike" : "arcade" },
        hud: {
          timeMs: S.timeMs,
          wave: S.wave,
          kills: S.kills,
    ...
    _resetRun() {
      ...
      S.player.armor = 0;
      S.player.plates = rogue ? CFG.player.platesStart : 0;
    ...
    _tickFixed(dt, tms) {
      ...
      // HARD: if something accidentally forces arcade while UI expects rogue, auto-heal to rogue mid-run.
      // (This is the bug that causes "kills exist but coins stay 0".)
      const rogue = isRogue(this);
      if (rogue && S.mode !== "roguelike") S.mode = "roguelike";
      if (!rogue && S.mode !== "arcade") S.mode = "arcade";
          ...
          if (dist < ((z.r || CFG.zombie.radius) + b.r)) {
            ...
            if (isBoss && typeof this._onBulletHit === "function") {
              ...
              if (z.hp <= 0) {
                ...
                // GUARANTEED economy tick in roguelike (no more "0 coins while killing")
                if (isRogue(this)) {
                ...
                if (isRogue(this)) this._dropOnKill(z, false);
      ...
      if (isRogue(this)) this._tickPickups(dt, tms);
  ...
  console.log("[Z_CORE] loaded (ULTRA LUX COD-ZOMBIES-2D v1.4 | 3D-READY CORE MAX++)");
})();
