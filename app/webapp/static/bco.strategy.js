/* BLACK CROWN OPS v30 — Premium Adaptive Strategy; recommendation, not player fact */
(() => {
  "use strict";
  if (window.__BCO_STRATEGY_V30_LOADED__) return;
  window.__BCO_STRATEGY_V30_LOADED__ = true;
  ...
  function mount() {
    ...
    if (!pane || $("#bcoStrategyV30")) return;
    const card = document.createElement("section");
    card.id = "bcoStrategyV30";
    card.className = "bco-op-card";
    card.innerHTML = `
      <div class="bco-op-title"><span>ADAPTIVE STRATEGY</span><small>PREMIUM / EVIDENCE-DRIVEN</small></div>
  ...
  function render(data) {
    ...
    meta.textContent = `${String(data?.strategy_class || "calibration").toUpperCase()} • CONF ${String(data?.confidence || "unknown").toUpperCase()} • ASSOCIATION ≠ CAUSATION • RECOMMENDATION ≠ FACT`;
    body.prepend(meta);
  }
  ...
  async function refresh() {
    ...
    if (status) status.textContent = "Building bounded strategy from server-authorized history…";
    try {
      ...
      if (status) status.textContent = "SERVER PREMIUM VERIFIED // STRATEGY IS A RECOMMENDATION, NOT A PLAYER FACT";
      render(payload.data || {});
    } catch (error) {
