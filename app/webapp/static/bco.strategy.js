/* BLACK CROWN OPS v31 — Premium Strategy Outcome Loop; Premium Adaptive Strategy; recommendation, not player fact */
(() => {
  "use strict";
  ...
  function initData() { return String(safe(() => window.Telegram?.WebApp?.initData, "") || "").trim(); }
  ...
  function mount() {
    ...
    card.innerHTML = `<div class="bco-op-title"><span>ADAPTIVE STRATEGY</span><small>PREMIUM / OUTCOME LOOP</small></div><div id="bcoStrategyStatus" class="bco-op-review">Resolve strategy on demand.</div><div id="bcoStrategyBody"></div><button id="bcoStrategyRefresh" class="bco-op-btn" type="button">BUILD NEXT OBJECTIVE</button>`;
  ...
  function render(data) {
    ...
    body.append(block("FOCUS", String(data?.focus || "calibration").replaceAll("_", " ").toUpperCase()), block("OBJECTIVE", data?.objective), block("SUCCESS CONDITION", data?.success_condition), block("NEXT ADAPTATION", data?.next_adaptation), block("RATIONALE", data?.rationale), block("OUTCOME LOOP", effectivenessText(data?.effectiveness)));
    const meta = document.createElement("div"); meta.className = "bco-op-flow";
    meta.textContent = `${String(data?.strategy_class || "calibration").toUpperCase()} • CONF ${String(data?.confidence || "unknown").toUpperCase()} • STRATEGY ${String(data?.strategy_id || "untracked").toUpperCase()} • ASSOCIATION ≠ CAUSATION • RECOMMENDATION ≠ FACT • EFFECTIVENESS ≠ CAUSAL PROOF`;
    body.prepend(meta);
  }
