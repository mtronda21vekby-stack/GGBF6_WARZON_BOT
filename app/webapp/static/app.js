...
(() => {
  ...
  window.__BCO_APP_BOOT_PROMISE__ = Promise.all([
    ...
    .then(() => loadScript("/webapp/bco.strategy.js", "__BCO_STRATEGY_V35_LOADED__").catch((error) => {
      window.__BCO_STRATEGY_V35_LOADED__ = false;
      console.warn("[BCO v35] Player Regime Detection unavailable; existing intelligence surfaces remain active", error);
      return false;
    }))
    .then(() => true)
    .catch((error) => {
