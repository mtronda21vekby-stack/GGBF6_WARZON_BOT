      ...
      for (let bi = S.bullets.length - 1; bi >= 0; bi--) {
        ...
        for (let zi = S.zombies.length - 1; zi >= 0; zi--) {
          ...
          if (dist < ((z.r || CFG.zombie.radius) + b.r)) {
            ...
            // If boss died (hook or fallback), remove + reward + drops (SAFE removal)
            if (isBoss && z.hp <= 0) {
              ...
              if (this.meta.mode === "roguelike") {
                ...
                this._spawnPickup(
                  ...
                  z.x - 18,
                  z.y,
                  { weaponKey: pick(Object.keys(CFG.weapons)), rarity: rollRarity(S.wave + 6), upgrade: clamp(Math.floor((S.wave - 1) / 5), 1, 4) }
                );
