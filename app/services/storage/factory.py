...
def build_store(settings: Any):
    ...
    try:
        ...
        primary = CanonicalReadShadowStore(
            persistent,
            enabled=bool(getattr(settings, "canonical_read_shadow_enabled", True)),
            sample_rate=float(getattr(settings, "canonical_read_shadow_sample_rate", 1.0) or 0.0),
            identity_cache_ttl_s=float(getattr(settings, "canonical_read_identity_cache_ttl_s", 120.0) or 120.0),
            identity_negative_cache_ttl_s=float(getattr(settings, "canonical_read_identity_negative_cache_ttl_s", 5.0) or 5.0),
            identity_cache_max_entries=int(getattr(settings, "canonical_read_identity_cache_max_entries", 10_000) or 10_000),
        )
        ...
        log.info(
            ...
            "outbox_max=%d replay_batch=%d canonical_read_shadow=%s sample_rate=%.3f",
            credential_kind, outbox_max, replay_batch, primary.enabled, primary.sample_rate,
        )
