...
class SupabaseStore:
    ...
    def _headers(self, extra: Mapping[str, str] | None = None) -> dict[str, str]:
        headers = {"apikey": self.key, "Authorization": f"Bearer {self.key}", "Accept": "application/json", "Content-Type": "application/json", "Accept-Profile": self.schema, "Content-Profile": self.schema, "User-Agent": "BLACK-CROWN-OPS/storage-v38"}
    ...
    def _request(self, method: str, path: str, *, params: Mapping[str, Any] | None = None, json: Any = None, extra_headers: Mapping[str, str] | None = None) -> httpx.Response:
        response = self._client.request(method, f"{self.rest_url}/{path.lstrip('/')}", params=dict(params or {}), json=json, headers=self._headers(extra_headers))
    ...
    def ping(self) -> bool:
        self._request("HEAD", "bco_players", params={"select": "chat_id", "limit": "1"}, extra_headers={"X-BCO-Storage-Probe": "startup-v38"}); return True
    ...
    def _append(self, table: str, payload: dict[str, Any], operation_id: str | None = None) -> None:
        ...
        if op:
            ...
            self._request("POST", table, params={"on_conflict": "operation_id"}, json=body, extra_headers={"Prefer": "resolution=ignore-duplicates,return=minimal"}); return
    ...
    @staticmethod
    def _mistake_key(mistake:str)->str:return hashlib.sha1(" ".join(str(mistake or "").lower().split()).encode()).hexdigest()[:20]
    ...
    def list_episodes(self,chat_id:int,limit:int=20)->list[dict]:
        rows=self._rows(self._request("GET","bco_episodes",params={"chat_id":f"eq.{int(chat_id)}","select":"kind,data,created_at","order":"id.desc","limit":str(max(1,min(int(limit),100)))})); return [dict(x.get("data") or {},kind=x.get("kind"),created_at=x.get("created_at")) for x in rows]
    ...
    # v38 server-side product analytics. These RPCs are service-role only.
    def record_user_activity(self,user_id:int,chat_id:int,*,language:str="",surface:str="telegram",is_message:bool=False,is_voice:bool=False,is_miniapp:bool=False)->None:
        self._request("POST","rpc/bco_record_user_activity",json={"p_user_id":int(user_id),"p_chat_id":int(chat_id),"p_language":str(language or "")[:16],"p_surface":str(surface or "telegram")[:32],"p_is_message":bool(is_message),"p_is_voice":bool(is_voice),"p_is_miniapp":bool(is_miniapp)},extra_headers={"Prefer":"return=minimal"})
    def admin_usage_summary(self)->dict[str,int]:
        rows=self._rows(self._request("POST","rpc/bco_admin_usage_summary",json={})); return dict(rows[0]) if rows else {}
