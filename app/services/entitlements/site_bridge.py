...
class SiteSessionVerifier:
    """Independently asks blackcrown.work to verify its signed session cookie."""

    def __init__(self, settings: Any, *, client: httpx.AsyncClient | None = None):
        raw_url = str(
            getattr(settings, "blackcrown_session_verify_url", "")
            or _DEFAULT_VERIFY_URL
        ).strip()
...
class SiteEntitlementBridgeAPI:
    ...
    async def _identity(self, token: str | None, site_user: str | None) -> str:
        return await self.verifier.verify(
            token=str(token or ""),
            expected_user_id=str(site_user or ""),
        )
    ...
    def _bind_routes(self) -> None:
        ...
        async def complete_link(
            ...
            try:
                site_user_id = await self._identity(x_bc_session_token, x_bc_site_user)
                ...
                log.warning("site session verification failed error=%s", type(exc).__name__)
