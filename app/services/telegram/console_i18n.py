...
def localize_view(view: Any, profile: Mapping[str, Any] | None) -> Any:
    ...
    try: return type(view)(text=text, reply_markup=markup)
    except Exception: return view

def _wrap(original):
    if getattr(original, "_bco_i18n_v41_wrapped", False): return original
    @wraps(original)
...
def install() -> None:
    ...
    try:
        ...
        for name, wrapped in wrapped_by_name.items():
            if hasattr(controller, name): setattr(controller, name, wrapped)
    except Exception:
        pass
