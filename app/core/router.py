    ...
    async def handle_update(self, update: Dict[str, Any]) -> None:
        ...
        if text in ("/start", "/menu", "Меню", "📋 Меню"):
            ...
            await self._send_main(
                chat_id,
                "🎁 Добро пожаловать в FPS Coach Bot (Warzone / BO7 / BF6 + Zombies)\n"
                "Premium UI снизу закреплён 👇\n\n"
                f"Текущий стиль: {vv} (по умолчанию — 🤝 Тиммейт)\n\n"
                "🤝 Тиммейт — максимально живой и открытый, но умный: разбор + план + микро-правки.\n"
                "📚 Коуч — элитный режим: структура, чек-листы, метрики, контроль прогресса.\n\n"
                "Напиши одной строкой:\n"
                "Игра | input | где чаще умираешь | что хочешь прокачать\n"
                "И я дам план как для демона 😈",
            )
            return
