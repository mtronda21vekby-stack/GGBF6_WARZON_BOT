...
class Settings(BaseSettings):
    ...
    # Phase 2C1 canonical read parity. This path always returns the legacy value.
    canonical_read_shadow_enabled: bool = _env_on("CANONICAL_READ_SHADOW_ENABLED")
    canonical_read_shadow_sample_rate: float = float(os.getenv("CANONICAL_READ_SHADOW_SAMPLE_RATE", "1.0"))
    canonical_read_identity_cache_ttl_s: float = float(os.getenv("CANONICAL_READ_IDENTITY_CACHE_TTL_S", "120"))
    canonical_read_identity_negative_cache_ttl_s: float = float(os.getenv("CANONICAL_READ_IDENTITY_NEGATIVE_CACHE_TTL_S", "5"))
    canonical_read_identity_cache_max_entries: int = int(os.getenv("CANONICAL_READ_IDENTITY_CACHE_MAX_ENTRIES", "10000"))
