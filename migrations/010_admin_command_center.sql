...
as $function$
  select jsonb_build_object(
    ...
    'users', jsonb_build_object(
      ...
      'new_24h', (select count(*) from public.bco_user_activity where first_seen_at >= now() - interval '24 hours'),
      'new_7d', (select count(*) from public.bco_user_activity where first_seen_at >= now() - interval '7 days')
    ),
