...
-- contract for unresolved/conflicting legacy ownership. The review workflow is
-- staged disabled by default and deliberately cannot:
...
create or replace function public.black_crown_set_ownership_review_enabled(
  p_enabled boolean,
  p_reason text
)
returns jsonb
...
begin
  if p_enabled is null then
    raise exception using
      errcode = '22023',
      message = 'ownership review enabled state is required';
  end if;

  if length(v_reason) < 8 or length(v_reason) > 256 then
    raise exception using
      errcode = '22023',
      message = 'ownership review flag reason is required';
  end if;

  insert into public.black_crown_ownership_runtime_flags as flags (
    flag_key,
    enabled,
    reason,
    updated_at
  ...
  returning flags.updated_at into v_updated_at;

  return jsonb_build_object(
    'schema_version', 'bco-ownership-resolution-v1',
    'review_enabled', p_enabled,
...
revoke all on function public.black_crown_set_ownership_review_enabled(
  boolean, text
) from public, anon, authenticated;
grant execute on function public.black_crown_set_ownership_review_enabled(
  boolean, text
) to service_role;
...
begin
  ...
  into v_review_enabled
  from public.black_crown_ownership_runtime_flags as flags
  where flags.flag_key = 'ownership_resolution_review';

  select jsonb_build_object(
    'schema_version', 'bco-ownership-resolution-v1',
    'review_enabled', coalesce(v_review_enabled, false),
    'total_cases', count(*),
...
begin
  ...
  if not coalesce(v_review_enabled, false) then
    raise exception using
      errcode = '55000',
      message = 'ownership review workflow is disabled';
  end if;
...
comment on function public.black_crown_begin_ownership_confirmation(
  ...
  'Creates a pending confirmation proposal only when the staged review flag is enabled. Does not write product owners, identities, accounts or entitlements.';
